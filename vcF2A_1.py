import os
import brille #brille 0.5.7 requried
import spglib
import numpy as np
...
lattice_pri, positions_direct, atom_types = spglib.standardize_cell(cell_given, to_primitive=True, no_idealize=False, symprec=1e-5)
lattice_con, _, _ = spglib.standardize_cell(cell_given, to_primitive=False, no_idealize=False, symprec=1e-5)

# hereafter we always use standard description of the cell
cell_std = (lattice_pri, positions_direct, atom_types)
symmetry_data = spglib.get_symmetry_dataset(cell_std)
...
# obtain the transformation matrix from the user's lattice to spglib lattice
user2std = decimal2fraction(np.dot(np.linalg.inv(lattice_given_3d), lattice_pri))
std2user = decimal2fraction(np.dot(np.linalg.inv(lattice_pri), lattice_given_3d))
...
for indicator in lte_list:
    grids_size = np.loadtxt(indicator+"/grid.dat", dtype=np.int64, comments=['#', '$', '@'], usecols=range(dim), max_rows=1)
    ...
    with open(indicator+"/force.dat",'r') as f:
        for line in f:
    ...
    with open(indicator+"/delta_prim.dat",'r') as f:
        if dim == 2:
            for line in f:
...
# Convert the coordinate of the input q points from the primitive lattice (user used) to the conventional lattice (Brille used)
q_points_con = np.dot(np.dot(q_points_3d, user2std), pri2con)
...
fig = plt.figure()
ax = Axes3D(fig)
ax.scatter(boundary[:,0], boundary[:,1], boundary[:,2], marker='o', color='salmon')
ax.scatter(q_points[:, 0], q_points[:, 1], q_points[:, 2], marker='x', color='gray')
ax._axis3don = False
plt.show()
"""
...
print(q_points_savt_direct)
print(savt_point_indicators)
...
for point, voronoi_cell in voronoi_cells.items():
    for s in voronoi_cell.simplices:
        cell = Poly3DCollection(voronoi_cell.points[s]) #Matplot 3.5.0 requried
...
for i, q_interp_ibz_pri in enumerate(q_target_ibz_pri):
    ...
    if overlap.size == 0:
        ...
        for point, voronoi_cell in voronoi_cells_ref.items():
            ...
            try:
                ...
                #sometimes 'point' in 'voronoi_cells_ref' is not in 'voronoi_cells_interp'. This means voronoi_cells_interp[point].volume = 0
                print(str(np.dot(q_interp_ibz_direct, std2user)) + " is not in voronoi_cells_interp")
                points_weight[index] = voronoi_cell.volume
    ...
    print(np.dot(q_interp_ibz_direct, std2user))
    print(grids_weight)
    with open(grid_target +  "/dyn_mat." + str(i+1) + ".dat", 'a') as f:
        for i_atom in range(num_atoms):
            for i_cart in range(3):
                for j_atom in range(num_atoms):
                    for j_cart in range(3):
                        f.write("%d %d %d %d %.17g %.17g\n" %(i_atom+1, i_cart+1, j_atom+1 ,j_cart+1, -np.real(D_q[i_atom, i_cart, j_atom, j_cart]), -np.imag(D_q[i_atom, i_cart, j_atom, j_cart])))
    f.close()
